...
def measure_dirs(dirA, dirB, use_gpu, logger=None, verbose=False):
    if verbose:
    ...
    for pathA, pathB in zip(paths_A, paths_B):
        ...
        logger.info(f"{pathA.split('/')[-1]}, {pathB.split('/')[-1]}, {format_result(**result)}, {d:0.1f}")
    ...
    logger.info(f"Final Result(PSNR, SSIM, LPIPS): {format_result(psnr, ssim, lpips)}, {time.time() - t_init:0.1f}s")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-dirA', default='C:/Projects/Datasets/LOLdataset/eval/eval15/high/', type=str)
    parser.add_argument('-dirB', default='C:/Projects/Datasets/LOLdataset/eval/eval15/high/', type=str)
    parser.add_argument('-type', default='png')
    parser.add_argument('--use_gpu', default=True)
    args = parser.parse_args()
    
    
    logging.basicConfig(filename=os.path.join(args.dirB, "eval_lol.log"), 
    ...
    logger.info(f"Comparing {dirA} and {dirB}")
    logger.info(f"Using GPU: {use_gpu}")
    logger.info(f"Image type: {type}")

    if len(dirA) > 0 and len(dirB) > 0:
        measure_dirs(dirA, dirB, use_gpu=use_gpu, logger=logger, verbose=True)